...
public class OrdineDAOImpl implements OrdineDAO {
	...
	public List<Ordine> findAllWIthArticoloWithCategoria(Categoria input) {
		Query q = entityManager.createNativeQuery(
				"select distinct o.id from ordine o INNER JOIN articolo a on o.id=a.ordine_id INNER JOIN articolo_categoria ac on a.id=ac.articolo_id INNER JOIN categoria c on c.id=ac.categoria_id where c.id= :idCat");
	...
	public List<String> findIndirizziConStringaNelNumeroSerialeArticoli(int input) {
		Query q = entityManager.createNativeQuery(
				"select distinct o.indirizzoSpedizione from ordine o INNER JOIN articolo a ON o.id=a.ordine_id INNER JOIN articolo_categoria ac ON a.id=ac.articolo_id INNER JOIN categoria c ON ac.categoria_id=c.id where a.numeroseriale LIKE ?");
